# Pricing Auth-Aware CTAs Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Show logged-in users contextual "Upgrade / Downgrade / Current plan" CTAs on the pricing page instead of static sign-up links.

**Architecture:** Split the existing `"use client"` pricing page into a server component wrapper (fetches session + workspace plan) and a `PricingPageClient` component. A new `PlanCTA` component encapsulates all CTA logic: disabled for current plan, checkout for new subscriptions, portal redirect for plan changes.

**Tech Stack:** Next.js App Router (server components), Lucia auth (`getSession`), `ensureUser` helper, Stripe checkout + portal APIs, Vitest, `useToast` from `@/components/ui/Toast`.
...
| Action | Path | Purpose |
|---|---|---|
| Modify | `app/(marketing)/pricing/page.tsx` | Convert to async server component; fetch session + plan; render client |
| Create | `app/(marketing)/pricing/PricingPageClient.tsx` | All existing client UI, accepts `userPlan` + `hasSubscription` props |
| Create | `components/marketing/PlanCTA.tsx` | CTA button logic (current / checkout / portal) |
| Create | `lib/__tests__/planCta.test.ts` | Unit tests for CTA decision logic |
| Create | `lib/planCtaLogic.ts` | Pure function: derives CTA label + action from plan state |

---

### Task 1: Pure CTA logic + tests

**Files:**
- Create: `lib/planCtaLogic.ts`
- Create: `lib/__tests__/planCta.test.ts`

The entire decision tree lives in one pure function so it can be unit-tested without DOM.
...
```bash
npx vitest run lib/__tests__/planCta.test.ts
```

Expected: `Cannot find module '@/lib/planCtaLogic'`
...
```bash
git add lib/planCtaLogic.ts lib/__tests__/planCta.test.ts
git commit -m "feat: add planCtaLogic pure function with tests"
```

---

### Task 2: PlanCTA component

**Files:**
- Create: `components/marketing/PlanCTA.tsx`
...
Expected: no errors related to `PlanCTA.tsx` or `planCtaLogic.ts`

- [ ] **Step 3: Commit**

```bash
git add components/marketing/PlanCTA.tsx
git commit -m "feat: add PlanCTA component for auth-aware pricing CTAs"
```

---

### Task 3: Extract PricingPageClient

**Files:**
- Create: `app/(marketing)/pricing/PricingPageClient.tsx`
- Modify: `app/(marketing)/pricing/page.tsx`

- [ ] **Step 1: Create PricingPageClient.tsx**

Create `app/(marketing)/pricing/PricingPageClient.tsx` — this is the entire current `page.tsx` content, but converted into a named export that accepts `userPlan` and `hasSubscription`, and uses `PlanCTA` instead of the hardcoded `<Link><Button>` CTA:
...
```bash
git add app/(marketing)/pricing/PricingPageClient.tsx
git commit -m "feat: extract PricingPageClient with userPlan + hasSubscription props"
```

---

### Task 4: Convert page.tsx to server component wrapper

**Files:**
- Modify: `app/(marketing)/pricing/page.tsx`

Replace the entire file content with a server component that fetches the session/plan and delegates to `PricingPageClient`.

- [ ] **Step 1: Replace page.tsx**

Replace the full content of `app/(marketing)/pricing/page.tsx` with:
...
git commit -m "feat: make pricing page auth-aware — upgrade/downgrade CTAs for logged-in users"
```
...
Open `http://localhost:3000/pricing` in a private/incognito window.

Expected:
- All four plan cards show the original CTA text ("Start free", "Start Basic", "Start Pro", "Start Agency")
- Clicking any paid plan CTA links to `/signup?plan=X`
- Clicking "Start free" links to `/signup`

- [ ] **Step 3: Test as logged-in FREE user**

Log in with a FREE-tier account, then navigate to `/pricing`.

Expected:
- FREE card: disabled "Current plan" button
- BASIC/PRO/AGENCY cards: "Upgrade to Basic / Pro / Agency" buttons
- Clicking an upgrade button → redirects to Stripe Checkout

- [ ] **Step 4: Test as logged-in paid user**

Log in with a paid account (e.g. PRO), then navigate to `/pricing`.

Expected:
- PRO card: disabled "Current plan" button
- AGENCY card: "Upgrade to Agency" button → redirects to Stripe Portal
- BASIC card: "Downgrade to Basic" button → redirects to Stripe Portal
- FREE card: "Downgrade to Free" button → redirects to Stripe Portal
...
Expected: all existing tests plus the new `planCta` tests pass
