...
export async function GET() {
  ...
  for (let i = 13; i >= 0; i--) {
    ...
    const sent = sentMessages.filter(
      (m: typeof sentMessages[number]) => m.sentAt != null && m.sentAt.toISOString().split("T")[0] === dateStr
    ).length;
    const replied = replyEvents.filter(
      (e: typeof replyEvents[number]) => e.createdAt.toISOString().split("T")[0] === dateStr
    ).length;
